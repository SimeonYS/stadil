...
class StadilPipeline:

    # Database setup
    conn = sqlite3.connect('stadil.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `stadil`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM stadil WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO stadil (date, title, link, content)"
